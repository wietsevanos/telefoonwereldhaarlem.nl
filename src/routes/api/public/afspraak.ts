    ...
    handlers: {
      ...
      POST: async ({ request }) => {
        try {
          ...
          if (!naam || !email || !telefoon || !repair || !datum || !tijd) {
          ...
          let slotLabel = `${datum} om ${tijd}`;
          const parsed = new Date(`${datum}T${tijd}:00`);
          if (!isNaN(parsed.getTime())) {
            slotLabel = parsed.toLocaleString("nl-NL", {
          ...
          const html = `
            ...
            <table cellpadding="6" style="font-family:Arial,sans-serif;font-size:14px;border-collapse:collapse">
              <tr><td><b>Gewenste datum & tijd</b></td><td>${esc(slotLabel)}</td></tr>
          ...
          const confirmHtml = `
            <div style="font-family:Arial,sans-serif;font-size:15px;color:#1a1a1a;max-width:560px;margin:0 auto;padding:24px">
              ...
              <p>We hebben uw aanvraag voor <b>${esc(slotLabel)}</b> ontvangen. We nemen zo snel mogelijk contact met u op om de afspraak definitief te bevestigen.</p>
